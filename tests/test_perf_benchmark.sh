...
if HELP_OUTPUT=$("$SCRIPT" --help 2>&1) &&
   ...
   printf '%s\n' "$HELP_OUTPUT" | grep -q -- '--history-records'; then
    pass "benchmark exposes budget and history controls"
else
    fail "benchmark help surface"
fi

BAD_OUTPUT=$("$SCRIPT" --startup-samples 4 --handshake-samples 5 \
    --fanout-samples 5 2>&1)
BAD_STATUS=$?
if [ "$BAD_STATUS" -ne 0 ] &&
...
DIAG_OUTPUT=$(
    "$SCRIPT" --binary "$DIAGNOSTIC.missing" --output "$DIAGNOSTIC" \
        ...
        --clients 1 --messages 1 --history-records 1 2>&1
)
DIAG_STATUS=$?
